...
#if ((defined CONFIG_VFS_TRACE_BIO_READ) || (defined CONFIG_VFS_TRACE_BIO_WRITE))
	{
			if (rq->bio) {
				...
				char * fname;
				char * ss;
				if (bio_op(rq->bio) == READ) { // 必须使用读，因为写有多种情况
#ifdef CONFIG_VFS_TRACE_BIO_READ
					if (rq->bio->bi_io_vec && rq->bio->bi_io_vec->bv_page
							&& rq->bio->bi_vcnt > 0) {
						bio_for_each_segment_all(bvec, rq->bio, i)
						{
							...
							if (page->mapping) {
								...
								if (inode) {
									if (inode->i_dentry.first) {
										...
										ss = dentry_path_raw(dentry, fname, VT_PATH_LEN);
										if (ss) {
											if (TRACE_CONDITION) {
											do_gettimeofday(&tv);
											printk(
												KERN_ALERT "[vfs]%ld,br,%s,%u,%u,%lu,%lld,%lu\n",
												(tv.tv_sec * 1000 + tv.tv_usec / 1000),
												ss, rq->bio->bi_iter.bi_size,
												rq->bio->bi_vcnt,
...
#ifdef CONFIG_VFS_TRACE_BIO_WRITE
					if (rq->bio->bi_io_vec && rq->bio->bi_io_vec->bv_page
							&& rq->bio->bi_vcnt > 0) {
						bio_for_each_segment_all(bvec, rq->bio, i)
						{
								...
								if (inode) {
									if (inode->i_dentry.first) {
										...
										if (ss) {
											...
											do_gettimeofday(&tv);
											printk(
												KERN_ALERT "[vfs]%ld,bw,%s,%u,%u,%lu,%lld,%lu\n",
												(tv.tv_sec * 1000 + tv.tv_usec / 1000),
												ss, rq->bio->bi_iter.bi_size,
												rq->bio->bi_vcnt,
												rq->bio->bi_iter.bi_sector,
