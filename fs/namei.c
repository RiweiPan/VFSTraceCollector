...
retry_deleg:
	...
	if (!IS_ERR(dentry)) {
		...
		if (error)
			goto exit2;
#ifdef CONFIG_VFS_TRACE_UNLINK
		{
			// should collect before unlink
			struct timeval tv;
			char *fname;
			char *ss;
			if(inode && dentry) {
				...
				if(fname) {
					...
					if(ss) {
						if(TRACE_CONDITION) {
							do_gettimeofday(&tv);
							// notice: ctime is not creation time, ctime = change time.
							printk(KERN_ALERT "[vfs]%ld,ul,%s,%ld,%ld,%ld,%lld,%lu\n",
															(tv.tv_sec * 1000 + tv.tv_usec / 1000),
															ss,
															(inode->i_ctime.tv_sec * 1000 + inode->i_ctime.tv_nsec / 1000000),
															(inode->i_atime.tv_sec * 1000 + inode->i_atime.tv_nsec / 1000000),
															(tv.tv_sec * 1000 + tv.tv_usec / 1000), inode->i_size, inode->i_ino);
						}
					}
					kfree(fname);
				}
